...
class RayArguments:
    ...
    base_dataset: Optional[str] = field(
        default=None,
        metadata={"help": "The dataset to use for base training. Default is None."},
    )
    base_num_samples: Optional[int] = field(
        default=None,
        metadata={"help": "The number of samples to use for base training. Default is None."},
    )
    base_num_steps: Optional[int] = field(
        default=None,
        metadata={"help": "The number of steps to use for base training. Default is None."},
    )
    base_threshold: Optional[float] = field(
        default=None,
        metadata={"help": "The threshold for gradient angle (in radians) to apply orthogonal projection. Default is None."},
    )
    base_scale: Optional[float] = field(
        default=1.0,
        metadata={"help": "The scale for gradient projection. Default is 1.0."},
    )
    base_method: Optional[Literal["orthogonal_projection", "threshold"]] = field(
        default="orthogonal_projection",
        metadata={"help": "The method for gradient projection: 'orthogonal_projection' (always project) or 'threshold' (project only when angle > threshold). Default is 'orthogonal_projection'."},
    )
